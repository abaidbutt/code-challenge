...
export async function listPokemon(limit: number): Promise<PokemonList> {
	try {
		return await typedFetch<PokemonList>(`https://pokeapi.co/api/v2/pokemon?limit=${limit}&offset=0`);
	} catch (error) {
...
export async function getPokemon(item: string | number): Promise<Pokemon> {
	try {
		return await typedFetch<Pokemon>(`https://pokeapi.co/api/v2/pokemon/${item}`);
	} catch (error) {
